...
The det-scan evidence consumer — the plugin side of the deterministic-filter program (Opengrep + gitleaks + Trivy behind the engine's default-off scan stage). The plugin never scans anything itself: it consumes the engine's redacted det-scan v1 evidence packets and turns each finding into a `det-scan:` needs_judgment scanner-evidence thread the review agents adjudicate — the scanner is evidence, the model stays the author of every verdict. The engine-side counterpart (the sandboxed scan stage, `OMNIFORGE_DET_SCAN` + `OMNIFORGE_DET_SCAN_SHADOW` knobs, ledger wiring, runbook) lands engine-side in the omniforge repo MR; this side is deliberately inert until the engine posts packets. The D0-2 calibration (314-MR corpus, three-pass adjudicated) narrowed what the engine will feed it to ~70-80 adjudicable findings corpus-wide, so the thread volume the review agents must judge stays bounded. Version 3.6.0 is the next minor above 3.5.2; the merge into main kept 3.5.2's corrected 23-MCP-tools description.

### Added
- **det-scan v1 packet consumer (`omni_det_scan.py`)** — strict top-level schema validation (unknown top-level fields REJECTED: contract drift is a hard failure, not a best-effort parse); posting is anchored through the existing get_all -> assemble_diff -> parse_diff_line_map chain so threads land on the full-MR diff truth (never --since-sha shapes); three guards: the packet's head-sha must match the MR head (stale packet -> skip with reason), an already-posted `det-scan:` summary note dedups the whole flow (one evidence set per head), and packets older than the review start are ignored.
- **`det-scan:` needs_judgment threads** — ONE summary note plus inline threads per finding, each carrying tool, rule_id, file:line, mapped severity, and the REDACTED preview verbatim from the packet — the plugin never sees or echoes raw scanner output. Threads are classified as `artifact` (never auto-resolved by disposition flows) and prefixed `det-scan:` so every downstream consumer can tell scanner evidence from agent findings; deliberately NO omniforge::* labels (Premium scoped labels sharing a key are mutually exclusive).
- **Downstream integration** — `omni_digest.py` classifies `det-scan:` notes so they ride the digest without displacing review prose; `omni_verdict.py` adds the prefix to `_ARTIFACT_BODY_MARKERS` so scanner evidence is exempt from verdict counting; `omni_prepare.py` grants the wipe exemption so prepared contexts keep det-scan threads.
- Test suite 926 + 128 subtests (was 876 + 128 at the 3.5.2 base); +50 tests (contract fixtures incl. unknown-field rejection, dedup + staleness guards, posting flow + severity mapping, artifact-exemption + wipe-exemption).

## [3.5.2] - 2026-09-16

Sweep hardening, the upload-boundary repair plus the selftest that proves it, and the screenshots activation floor. The engine-side counterpart — the sweep hardening consumption (chunked-call orchestration, session/timeout env forwarding) and the `OMNIFORGE_SCREENSHOTS` hook activation — lands engine-side in the omniforge repo MR; the engine gates the screenshots hook on **plugin >= 3.5.2** (`SCREENSHOTS_MIN_PLUGIN_VERSION=(3, 5, 2)` in the engine gateway), so this bump is the activation unlock for the 3.5.1 ask-the-author machinery. It also repairs a real break: every `upload_project_file` invocation since PR #39 died at argv-parse (glab exit 1) before any bytes reached GitLab.

### Added
- **Sweep chunking + call-path wiring (pairs with the engine-side MR)** — `omni_sweep.py`: batches over `CHUNK_THRESHOLD` open findings (default 15, knob `OMNIFORGE_SWEEP_CHUNK_THRESHOLD`) split ONCE into two contiguous halves (first `ceil(n/2)`, rest) and run as two sequential `claude -p` calls instead of one oversized payload; each chunk gets its OWN evidence budget (a fresh `MAX_PACKET_CHARS`), its own prompt, and its own call; the verdict merge is deterministic (chunk order), degradation is per chunk (a failed chunk-2 call degrades only chunk-2's findings to `needs_judgment` — chunk-1's verdicts stand), a cross-chunk shadowing guard drops any verdict naming a finding outside its own chunk before the merge (the report's by-id map is last-wins), and the citation rule is preserved — it runs ONCE over the merged verdict set against the concatenated packets of every chunk; at or below the threshold the sweep stays today's single byte-identical call. Session-id passthrough: the engine forwards `OMNIFORGE_SESSION_ID`; the first call rides it as `claude --session-id` (an explicit id arg wins over the env), each additional chunked call mints a fresh uuid4 session so the chunks' transcripts never collide, and the result JSON gains an additive `sessions` key (the ids actually used, in call order; `[]` on skip paths) — the transcripts are linkable per sweep. Provider timeout wiring: `ClaudeSpawnProvider`'s per-call timeout is now `OMNIFORGE_SWEEP_PLUGIN_TIMEOUT_SECS` + 60 s grace (replacing the hardcoded 300 s) so the ENGINE kill stays the binding ceiling whether its ceiling sits at today's 240 s or the spec-raised 600 s. `sweep-prompt.md` documents the per-chunk call path (the reply contract — a JSON array of verdict objects — is unchanged either way).
- **`upload_project_file --selftest` + the argv FIX that unblocks it** — the FIX first: `glab api -H` requires `Key: value` form, and PR #39 passed the bare header VALUE, which glab rejects locally (exit 1, "requires a value separated by ':'") — every upload since #39 was broken before any request left the box; `-H` now carries the full `Content-Type: multipart/form-data; boundary=…` header so the deterministic boundary rides the request. The selftest: `omniforge_mcp_server.py --selftest --selftest-project PATH` (explicit invoke only — the MCP server is NEVER started on this path) uploads a stdlib-built 1×1 PNG (zlib + crc32, no imaging deps) and prints EXACTLY one verdict line — `OK` (exit 0): upload succeeded AND the returned markdown matches the plugin's `![…](…/uploads/…)` upload shape; `STRIPPED` (exit 4): the call succeeded but the markdown is empty or malformed (the boundary header was lost, GitLab saw garbage); `FAILED` (exit 3): the call failed or raised. This retires PR #39's carried caveat ("whether glab preserves the multipart boundary header is unverified") with a first-try probe instead of hoping.
- **Screenshots state-machine hardening (engine activation support)** — `omni_ui_screenshot.py::decide_review_ask` now honors the same-round re-ask cap: when a push re-ask already re-armed the request this round (`decide_push_reask` fired after an image landed), review_ask plans `none` / `reask_capped_this_round` instead of re-opening the thread — the pre-existing `asked_round` cap cannot catch this case because a re-ask advances only `reasked_round`, leaving `asked_round` a round behind. Required before the engine drives the flow live.

### Changed
- **The fix brief is now ONE copyable fenced `markdown` block** — `omni_fixprompt.py::wrap_brief_md_block` wraps the composed brief as the LAST render step: the fence is always ≥ 4 backticks and one longer than the payload's longest backtick run (nothing inside the brief can close the block early), and the payload stays byte-identical — it must end in exactly one newline (asserted, never repaired), so there is no blank line before the closing fence; poster stdout is exactly the wrapped block, which gives the MR note GitLab's one-click copy. The digest carries the wrapped brief VERBATIM: `omni_digest.py::is_omniforge_note`'s header arm now also matches at any line start, so the brief's heading sitting one line below the opening fence still classifies as a bot artifact and rides uncapped in digest + prior-findings (never 500-char prose-truncated) — same marker matching as `omni_verdict.py` `is_artifact`, no fence parsing.
- plugin.json description tool count corrected: 19 → 23 MCP tools (the `@mcp_server.tool()` count at this HEAD).
- Test suite 876 + 128 subtests (was 840 + 128 at the 3.5.1 base); +36 tests (chunking/boundary/sessions/timeout, selftest verdicts + argv shape, re-ask cap, brief wrap + digest verbatim carry)
